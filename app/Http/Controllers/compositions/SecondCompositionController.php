...
class SecondCompositionController extends Controller
{
    public function index()
    {
        $classes = Classes::all();
        return view('compositions.quarters.second.index', [
            'classes' => $classes
        ]);
    }
    ...
    public function filteredCompositions(Request $request)
    {
        ...
        return datatables()->of($averages)
            ->addColumn('action', 'compositions.quarters.second.composition-action')
            ->rawColumns(['action'])
            ->make('true');
    }
    ...
    private function getResultsOfClass($classId)
    {
        ...
        foreach ($students as $student) {
            ...
            foreach ($subjects as $subject) {
                $secondTest = $query($student->id, 2, $subject->id, 'exams');
                $notes[$subject->id] = [
